...
Система поддерживает автоматическое обновление элементов каталога при редактировании постов в Telegram:

- **Если забыли добавить хештег** - просто отредактируйте пост в Telegram, добавьте хештег, и элемент каталога будет создан автоматически
- **Если изменили текст поста** - элемент каталога будет обновлен с новым текстом
- **Если изменили изображение** - изображение в элементе каталога будет обновлено
- **Если удалили хештег** - элемент каталога будет деактивирован (is_active=False)
- **Если удалили пост** - элемент каталога будет деактивирован (is_active=False)

Система отслеживает посты по `message_id` из Telegram, поэтому каждый пост связан с одним элементом каталога.

**Важно об удалении постов:**
Telegram Bot API не отправляет стандартные события об удалении сообщений через webhook. Система обрабатывает возможные форматы событий об удалении, которые могут прийти от некоторых реализаций или будущих версий API. Для надежной проверки удаленных сообщений рекомендуется:
- Использовать MTProto (Telethon) вместо Bot API
- Или деактивировать элементы вручную через админку Django
- Или использовать команду `check_deleted_telegram_posts` для периодической проверки (с ограничениями)
