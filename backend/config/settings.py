...
ALLOWED_HOSTS = config('ALLOWED_HOSTS', default='localhost,127.0.0.1,api.temis.ooo,temis.ooo', cast=lambda v: [s.strip() for s in v.split(',')])
...
MIDDLEWARE = [
    'corsheaders.middleware.CorsMiddleware',  # CORS должен быть первым для обработки всех запросов, включая ошибки
