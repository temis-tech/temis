...
class CatalogItemAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Страница элемента', {
            'fields': ('has_own_page', 'slug', 'description', 'image', 'page_image_preview', 'image_align', 'image_size', 'video_url', 'video_width', 'video_height'),
            'description': 'Настройки страницы элемента (отображается при открытии карточки, если включен режим "Может быть открыт как страница"). Здесь можно задать полное описание, изображение, видео (YouTube, Rutube и т.д.) с настройками размера и параметры отображения.'
        }),
        ('Настройки', {
