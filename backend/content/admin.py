...
class ContentPageAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Настройки каталога (для типа "Каталог")', {
            ...
            'description': 'Если включено, на странице каталога будет отображаться навигационный список элементов каталога (рубрикатор) для удобной навигации по статьям. Рубрикатор отображается только для элементов, у которых включен режим "Может быть открыт как страница".'
        }),
        ('Отображение', {
            'fields': ('show_title',),
...
class HeaderSettingsAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Размеры', {
            'fields': ('logo_height', 'logo_width', 'logo_mobile_scale', 'header_height'),
            'description': 'Высота шапки используется для расчета отступа контента, чтобы он не перекрывался фиксированной шапкой. Ширина логотипа ограничивает размер на мобильных устройствах. Уменьшение на мобильных позволяет автоматически уменьшить логотип на узких экранах.'
        }),
        ('Меню', {
