# 🚀 Настройка нового сервера - Пошаговая инструкция

## Данные сервера
- **IP:** `2a03:6f01:1:2::1:f3f5` (IPv6)
- **SSH:** `ssh root@2a03:6f01:1:2::1:f3f5`
- **Пользователь:** `root`
- **Пароль:** `mW6iYUw2^Fv2+g`

## Шаг 1: Создание нового репозитория на GitHub

1. Перейди на GitHub и создай новый репозиторий
2. **НЕ** создавай README, .gitignore или лицензию (у нас уже есть)
3. Скопируй URL репозитория (например: `git@github.com:username/temis.git`)

## Шаг 2: Настройка git remote

Выполни локально:

```bash
# Используй скрипт для настройки нового remote
./scripts/setup-new-repo.sh git@github.com:твой-username/temis.git

# Или вручную:
git remote remove origin  # если есть старый origin
git remote add origin git@github.com:твой-username/temis.git
```

## Шаг 3: Создание SSH ключа для деплоя

```bash
# Создай SSH ключ
ssh-keygen -t ed25519 -C "github-actions-deploy" -f ~/.ssh/temis_deploy
# Нажми Enter при запросе пароля (без пароля)

# Добавь публичный ключ на сервер
cat ~/.ssh/temis_deploy.pub | ssh root@2a03:6f01:1:2::1:f3f5 "mkdir -p ~/.ssh && chmod 700 ~/.ssh && cat >> ~/.ssh/authorized_keys && chmod 600 ~/.ssh/authorized_keys"

# Проверь подключение
ssh -i ~/.ssh/temis_deploy root@2a03:6f01:1:2::1:f3f5 "echo 'SSH работает!'"
```

## Шаг 4: Настройка GitHub Secrets

1. Перейди в репозиторий на GitHub: **Settings → Secrets and variables → Actions**
2. Добавь следующие secrets:

### `SSH_PRIVATE_KEY`
```bash
# Скопируй содержимое приватного ключа:
cat ~/.ssh/temis_deploy
```
Вставь весь вывод (включая `-----BEGIN OPENSSH PRIVATE KEY-----`)

### `SERVER_HOST`
```
2a03:6f01:1:2::1:f3f5
```

### `SERVER_USER`
```
root
```

## Шаг 5: Первоначальная настройка сервера

Подключись к серверу и выполни:

```bash
ssh root@2a03:6f01:1:2::1:f3f5

# Загрузи и выполни скрипт настройки
# (Скопируй содержимое scripts/setup-server-initial.sh на сервер)
```

Или выполни команды вручную (см. `scripts/setup-server-initial.sh`)

## Шаг 6: Пуш кода в новый репозиторий

```bash
# Добавь все изменения
git add .

# Создай коммит
git commit -m "Initial commit for new server"

# Запушь в новый репозиторий
git push -u origin master

# Если GitHub использует main вместо master:
git push -u origin master:main
```

## Шаг 7: Проверка деплоя

1. Перейди в раздел **Actions** на GitHub
2. Дождись завершения workflow
3. Проверь логи деплоя

## Шаг 8: Настройка домена и SSL

После успешного деплоя:

```bash
ssh root@2a03:6f01:1:2::1:f3f5

# Настрой DNS записи для твоего домена
# Затем получи SSL сертификаты:
sudo certbot --nginx -d твой-домен.com -d api.твой-домен.com

# Перезапусти nginx
sudo systemctl reload nginx
```

## Проверка работы

```bash
# Статус сервисов
sudo systemctl status temis-frontend
sudo systemctl status temis-backend

# Логи
sudo journalctl -u temis-frontend -f
sudo journalctl -u temis-backend -f
```

## Важные замечания

1. **IPv6 адрес:** Убедись, что твой интернет-провайдер поддерживает IPv6
2. **Порты:** Порты 53413, 2525, 25, 3389, 389, 587, 465 закрыты. Если нужно открыть другие - сообщи
3. **Домен:** Не забудь настроить DNS записи перед получением SSL
4. **.env файл:** После первого деплоя проверь и настрой `/var/www/temis/backend/.env`

## Полезные файлы

- `deploy/GITHUB_CI_CD_SETUP.md` - Подробная инструкция по CI/CD
- `scripts/setup-new-repo.sh` - Скрипт настройки git remote
- `scripts/setup-server-initial.sh` - Скрипт первоначальной настройки сервера
- `.github/workflows/ci.yml` - GitHub Actions workflow

