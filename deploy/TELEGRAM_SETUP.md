# Настройка Telegram бота на продакшене

## ✅ Что уже сделано

1. ✅ Приложение `telegram` задеплоено на сервер
2. ✅ Миграции применены (созданы таблицы и настройки с дефолтным токеном)
3. ✅ URL для webhook добавлен: `/api/telegram/webhook/`

## 📋 Следующие шаги

### 1. Подключись к серверу

```bash
ssh administrator@85.190.102.101
```

### 2. Установи webhook для бота

```bash
cd /var/www/temis/backend
sudo -u www-data ./venv/bin/python manage.py setup_webhook
```

Эта команда установит webhook на URL: `https://api.temis.estenomada.es/api/telegram/webhook/`

### 3. Проверь настройки бота в админке

1. Открой админку: `https://api.temis.estenomada.es/admin/`
2. Перейди в раздел "Настройки Telegram бота"
3. Убедись, что:
   - Токен установлен: `8576779956:AAEmqm1yQmtO06aiXdcdUfi-H4ZKqecbZBo`
   - Бот активен (включи галочку "Активен")
   - Настройки уведомлений включены

### 4. Активируй бота в Telegram

1. Найди своего бота в Telegram
2. Отправь команду `/start` или `/help`
3. В админке появится запись о новом пользователе в разделе "Пользователи Telegram"

### 5. Назначь администраторов

1. В админке перейди в раздел "Пользователи Telegram"
2. Найди пользователя, который должен получать уведомления
3. Отметь галочку **"Админ"**
4. Сохрани

**Важно:** Только пользователи с отмеченной галочкой "Админ" будут получать уведомления.

### 6. (Опционально) Настрой периодическую проверку баннеров

Добавь в cron задачу для проверки баннеров:

```bash
sudo crontab -e -u www-data
```

Добавь строку:

```
* * * * * cd /var/www/temis/backend && ./venv/bin/python manage.py check_banners
```

Это будет проверять баннеры каждую минуту и отправлять уведомления при начале/завершении отображения.

## 🔍 Проверка работы

### Проверь webhook

```bash
curl https://api.telegram.org/bot8576779956:AAEmqm1yQmtO06aiXdcdUfi-H4ZKqecbZBo/getWebhookInfo
```

Должен вернуть информацию о текущем webhook.

### Проверь логи

```bash
# Логи Django
sudo journalctl -u temis-backend -f

# Или логи в файле (если настроено)
tail -f /var/www/temis/backend/logs/*.log
```

## 🐛 Решение проблем

### Webhook не устанавливается

1. Проверь, что домен `api.temis.estenomada.es` доступен из интернета
2. Проверь, что SSL сертификат валиден
3. Проверь, что URL доступен: `curl https://api.temis.estenomada.es/api/telegram/webhook/`

### Уведомления не приходят

1. Проверь, что бот активен в админке
2. Проверь, что у пользователя отмечена галочка "Админ"
3. Проверь логи на ошибки
4. Проверь, что настройки уведомлений включены для нужных событий

### Бот не отвечает на команды

1. Проверь, что webhook установлен правильно
2. Проверь логи Django на наличие ошибок
3. Убедись, что URL webhook доступен из интернета

