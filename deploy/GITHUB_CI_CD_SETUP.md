# 🚀 Настройка CI/CD для нового сервера

Этот документ описывает настройку автоматического деплоя через GitHub Actions на новый сервер.

## Шаг 1: Создание SSH ключа для деплоя

На локальной машине создай SSH ключ специально для деплоя:

```bash
ssh-keygen -t ed25519 -C "github-actions-deploy" -f ~/.ssh/rainbow_say_deploy
```

**Важно:** Не устанавливай пароль на ключ (просто нажми Enter).

## Шаг 2: Добавление публичного ключа на сервер

Скопируй публичный ключ на новый сервер:

```bash
# Вариант 1: Используя ssh-copy-id (если установлен)
ssh-copy-id -i ~/.ssh/rainbow_say_deploy.pub root@2a03:6f01:1:2::1:f3f5

# Вариант 2: Вручную
cat ~/.ssh/rainbow_say_deploy.pub | ssh root@2a03:6f01:1:2::1:f3f5 "mkdir -p ~/.ssh && chmod 700 ~/.ssh && cat >> ~/.ssh/authorized_keys && chmod 600 ~/.ssh/authorized_keys"
```

Проверь подключение:

```bash
ssh -i ~/.ssh/rainbow_say_deploy root@2a03:6f01:1:2::1:f3f5 "echo 'SSH работает!'"
```

## Шаг 3: Настройка GitHub Secrets

1. Перейди в настройки репозитория на GitHub:
   - Settings → Secrets and variables → Actions

2. Добавь следующие secrets:

### `SSH_PRIVATE_KEY`
Содержимое приватного ключа:
```bash
cat ~/.ssh/rainbow_say_deploy
```
Скопируй весь вывод (включая `-----BEGIN OPENSSH PRIVATE KEY-----` и `-----END OPENSSH PRIVATE KEY-----`)

### `SERVER_HOST`
Адрес сервера:
```
2a03:6f01:1:2::1:f3f5
```

### `SERVER_USER`
Пользователь для SSH:
```
root
```

## Шаг 4: Первоначальная настройка сервера

Перед первым деплоем нужно настроить сервер. Выполни на сервере:

```bash
# Подключись к серверу
ssh root@2a03:6f01:1:2::1:f3f5

# Запусти скрипт первоначальной настройки
bash <(curl -s https://raw.githubusercontent.com/ваш-username/rainbow-say/master/scripts/setup-server-initial.sh)
```

Или вручную:

```bash
# 1. Установи необходимые пакеты
apt-get update
apt-get install -y python3 python3-pip python3-venv nodejs npm nginx git

# 2. Создай директорию для проекта
mkdir -p /var/www/rainbow-say
chown -R www-data:www-data /var/www/rainbow-say

# 3. Создай systemd сервисы (см. deploy/configs/systemd/)
# 4. Создай nginx конфигурацию (см. deploy/configs/nginx/)
# 5. Создай .env файл для бэкенда в /var/www/rainbow-say/backend/.env
```

## Шаг 5: Проверка деплоя

После настройки secrets и сервера:

1. Сделай коммит и пуш в ветку `master` или `main`
2. GitHub Actions автоматически запустит деплой
3. Проверь логи деплоя в разделе Actions на GitHub

## Структура деплоя

GitHub Actions workflow выполняет следующие шаги:

1. ✅ Проверяет бэкенд (миграции, проверка кода)
2. ✅ Проверяет фронтенд (линтер, сборка)
3. ✅ Собирает фронтенд (`npm run build`)
4. ✅ Создает архив с файлами
5. ✅ Загружает архив на сервер через SSH
6. ✅ Распаковывает файлы на сервере
7. ✅ Устанавливает зависимости
8. ✅ Выполняет миграции Django
9. ✅ Перезапускает systemd сервисы

## Устранение проблем

### Ошибка SSH подключения
- Проверь, что публичный ключ добавлен в `~/.ssh/authorized_keys` на сервере
- Проверь, что `SSH_PRIVATE_KEY` в GitHub Secrets содержит весь ключ
- Проверь, что `SERVER_HOST` и `SERVER_USER` указаны правильно

### Ошибка деплоя на сервере
- Проверь логи на сервере: `journalctl -u rainbow-say-backend -f`
- Проверь права доступа: `ls -la /var/www/rainbow-say`
- Убедись, что systemd сервисы созданы

### Проблемы с портами
Если нужно открыть дополнительные порты, сообщи хостинг-провайдеру или настрой файрвол:
```bash
# Проверка открытых портов
sudo ufw status
# Открытие порта (если используешь ufw)
sudo ufw allow 22/tcp
```

## Дополнительные настройки

### Переменные окружения для бэкенда

Создай файл `/var/www/rainbow-say/backend/.env` на сервере:

```env
SECRET_KEY=твой-секретный-ключ
DEBUG=False
ALLOWED_HOSTS=твой-домен.com,api.твой-домен.com
DATABASE_URL=sqlite:///db.sqlite3
# И другие переменные
```

### Настройка домена

После деплоя настрой DNS записи и получи SSL сертификаты:

```bash
sudo certbot --nginx -d твой-домен.com
sudo certbot --nginx -d api.твой-домен.com
```

